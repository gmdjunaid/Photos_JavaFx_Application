    ...
    private void copyPhotoToAlbum() {
        if (displayedPhoto != null) {
            ...
            if (result.isPresent()) {
                ...
                if (selAlbum.equalsIgnoreCase(currentAlbum.getAlbumName())) {
                    Alert alert = new Alert(Alert.AlertType.ERROR);
                    alert.setTitle("Error");
                    alert.setHeaderText("Cannot copy to the same album.");
                    alert.setContentText("Please select a different album.");
                    alert.showAndWait();
                } else {
                    for (Album album : loggedInUser.getUserAlbums()) {
                        if (album.getAlbumName().equalsIgnoreCase(selAlbum)) {
                            for (Photo photo : album.getPhotos()) {
                                if (photo.equals(displayedPhoto)) {
                                    Alert alert = new Alert(Alert.AlertType.ERROR);
                                    alert.setTitle("Error");
                                    alert.setHeaderText("This album already contains this photo.");
                                    alert.setContentText("Please select a different photo or album.");
                                    alert.showAndWait();
                                    return;
                                }
                            }
                    ...
                    if (album.getAlbumName().equalsIgnoreCase(selAlbum)) {
                        for (Photo photo : album.getPhotos()) {
                            if (photo.equals(displayedPhoto)) {
                                Alert alert = new Alert(Alert.AlertType.ERROR);
                                alert.setTitle("Error");
                                alert.setHeaderText("Cannot move a photo to an album that already contains it.");
                                alert.setContentText("Please select a different album.");
                                alert.showAndWait();
                                return;
                            }
        ...
        if (file != null) {
            try {
                ...
                for (Photo p : currentAlbum.getPhotos()) {
                    if (p.getFilePathToString().equalsIgnoreCase(photo.getFilePathToString())) {
                        Alert alert = new Alert(Alert.AlertType.ERROR);
                        alert.setTitle("Error");
                        alert.setHeaderText("This photo already exists in this album.");
                        alert.setContentText("Please select a different photo, or delete the existing photo.");
                        alert.showAndWait();
                        return;
                    }
                }
